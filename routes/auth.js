...
router.post('/register', async function (req, res, next) {
    // Đã gỡ bỏ session và transaction để chạy mượt trên localhost
    try {
        ...
        await newCart.save(); // Không dùng { session } nữa
...
router.post('/login', async function (req, res, next) {
    try {
        ...
        if (bcrypt.compareSync(password, user.password)) {
            user.loginCount = 0; // Đã sửa lỗi thiếu chữ 'user.' ở đây
            await user.save()
            let token = jwt.sign({
                id: user._id
            }, 'secret', {
            ...
            // res.send(token) - Nên gửi về object để Postman dễ đọc hơn
